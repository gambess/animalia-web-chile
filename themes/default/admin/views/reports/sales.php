...
if ($this->input->post('paying_actions')) {
    $v .= "&paying_actions=" . $this->input->post('paying_actions');
}
...
<script>
    $(document).ready(function () {
        oTable = $('#SlRData').dataTable({
            ...
            "rowCallback": function( row, data, index ) {/*datos null por pendientes*/
                if ( data.t_pago === '' ) {
                    $("td:eq(9)", row).addClass("active");
                }
            },
            ...
            "aoColumns": [{"mRender": fld}, null, null, null, {
                "bSearchable": false,
                "mRender": pqFormat
            }, {"mRender": currencyFormat}, {"mRender": currencyFormat}, {"mRender": currencyFormat}, {"mRender": row_status}, {"mRender":function (data) {
                    return data !== null ? data : "Pendiente";
            ...
            data:'t_pago', render: function(data , type , row){
            ...
            {column_number: 8, filter_default_label: "[<?=lang('payment_status');?>]", filter_type: "text", data: []},
            {
                ...
                data: [
                    ...
                    {value: '<?= $row->name ?>', label: ' <?= $row->name?$row->name:'pendiente' ?> '},
                    <?php endforeach; ?>
                    ]
            },
            ...
            <div class="col-lg-12">
                ...
                <div id="form">
                    ...
                    <div class="row">
                        ...
                        <div class="col-sm-4">
                            <div class="form-group">
                                <label class="control-label" for="type"><?= lang("Metodo de Pago"); ?></label>
                                <?php
                                $ty[""] = lang('select').' '.lang('Metodo de Pago');
                                foreach ($paying_by as $type) {
                                    $ty[$type->id] = $type->name;
                                }
                                echo form_dropdown('payment_type', $ty, (isset($_POST['payment_type']) ? $_POST['payment_type'] : ""), 'class="form-control" id="type" data-placeholder="' . $this->lang->line("select") . " " . $this->lang->line("Metodo de Pago") . '"');
                                ?>
                            </div>
                        </div>
                        ...
                        <div class="col-sm-4">
                            <div class="form-group">
                                <label class="control-label" for="type"><?= lang("Tipo de Venta"); ?></label>
                                <?php
                                $act[""] = lang('select').' '.lang('Tipo de Venta');
                                foreach ($paying_actions as $actions) {
                                    $act[$actions->id] = $actions->name;
                                }
                                echo form_dropdown('paying_actions', $act, (isset($_POST['paying_actions']) ? $_POST['paying_actions'] : ""), 'class="form-control" id="type" data-placeholder="' . $this->lang->line("select") . " " . $this->lang->line("Tipo de Venta") . '"');
                                ?>
                            </div>
                        </div>
                    </div>

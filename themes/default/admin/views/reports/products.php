...
<script>
    $(document).ready(function () {
        ...
        oTable = $('#PrRData').dataTable({
            ...
            "aoColumns": [null, null, {"mRender": spb}, {"mRender": spb}, {"mRender": currencyFormat}, {"mRender": spb}, {"mRender": currencyFormat}, {"mRender": currencyFormat},  {"mRender": formatQuantity},  {"mRender": currencyFormat}, {"bSortable": true}],
            "fnFooterCallback": function (nRow, aaData, iStart, iEnd, aiDisplay) {
                ...
                var pq = 0, sq = 0, bq = 0, pa = 0, sa = 0, ba = 0, pl = 0, ca = 0, st = 0;
                for (var i = 0; i < aaData.length; i++) {
                    p = (aaData[aiDisplay[i]][2]).split('__');
                    s = (aaData[aiDisplay[i]][3]).split('__');
                    b = (aaData[aiDisplay[i]][5]).split('__');
                    ...
                    ba += parseFloat(b[1]);
                    pl += parseFloat(aaData[aiDisplay[i]][4]);
                    st += parseFloat(aaData[aiDisplay[i]][8]);
                    ca += parseFloat(aaData[aiDisplay[i]][9]);
                }
                var nCells = nRow.getElementsByTagName('th');
                nCells[2].innerHTML = '<div class="text-right">('+formatQuantity2(pq)+') '+formatMoney(pa)+'</div>';
                nCells[3].innerHTML = '<div class="text-right">('+formatQuantity2(sq)+') '+formatMoney(sa)+'</div>';
                nCells[4].innerHTML = currencyFormat(parseFloat(pl));
                nCells[5].innerHTML = '<div class="text-right">('+formatQuantity2(bq)+') '+formatMoney(ba)+'</div>';
                nCells[9].innerHTML = '<div class="text-right">'+formatMoney(ca)+'</div>';
            }
            ...
            {column_number: 10, filter_default_label: "[<?=lang('company');?>]", filter_type: "text", data: []},
        ], "footer");
    });
</script>
            ...
            <div class="col-lg-12">
                    ...
                    <div class="row">
                        ...
                        <div class="col-sm-4">
                            <div class="form-group">
                                ...
                                <?php
                                $ca[''] = lang('select').' '.lang('company');
                                foreach ($companies as $company) {
                                    $ca[$company->id] = $company->company;
                                }
                                echo form_dropdown('company', $ca, (isset($_POST['company']) ? $_POST['company'] : ''), 'class="form-control select" id="category" placeholder="' . lang("select") . " " . lang("company") . '" style="width:100%"')
                                ?>
                            </div>
                        </div>
                    </div>
                ...
                <div class="table-responsive">
                    ...
                    <table class="table table-striped table-bordered table-condensed table-hover dfTable reports-table">
                        ...
                        <tr>
                            <td>Resumen</td>
                            <td><?= number_format($info_products->total, 2, '.', ',') ?></td>
                            <td><?= number_format($info_products->total_stock, 0, '', '') ?></td>
                        </tr>
                        </tbody>
                    </table>
                </div>

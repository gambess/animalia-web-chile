...
class Reports extends MY_Controller
{
    ...
    function __construct()
    {
         ...
         * Agregados
         */
        $this->load->admin_model('payment_type_model');
        $this->load->admin_model('payment_actions_model');
    ...
    function products()
    {
        ...
        $this->data['warehouses'] = $this->site->getAllWarehouses();
        $this->data['info_products'] = $this->reports_model->getProductsResum();
        $this->data['companies'] = $this->companies_model->getAllSupplierCompanies();
    ...
    function getProductsReport($pdf = NULL, $xls = NULL)
    {
        ...
        if ($start_date || $warehouse) {
            ...
            if ($start_date) {
                ...
                $pp .= " WHERE p.date >= '{$start_date}' AND p.date < '{$end_date}' ";
            ...
            if ($warehouse) {
                $pp .= " WHERE pi.warehouse_id = {$warehouse} ";
        ...
        if ($pdf || $xls) {
            ...
            $this->db
                ->select($this->db->dbprefix('products') . ".code, " . $this->db->dbprefix('products') . ".name ,COALESCE( " . $this->db->dbprefix('products') . ".cost, 0) as cost, COALESCE(". $this->db->dbprefix('products') . ".price, 0) as price, COALESCE(" . $this->db->dbprefix('products') . ".quantity, 0) as quantity, 
                ...
                (COALESCE( PSales.totalSale, 0 ) - COALESCE( PCosts.totalPurchase, 0 )) as Profit,COALESCE({$this->db->dbprefix('products')}.cost * {$this->db->dbprefix('products')}.quantity,  0) as total, {$this->db->dbprefix('companies')}.company as company, ", FALSE)
                ->from('products')
                ->join('companies', 'products.supplier1=companies.id', 'left')
                ->join($sp, 'products.id = PSales.product_id', 'inner')
                ->join($pp, 'products.id = PCosts.product_id', 'inner')
            ...
            if ($company) {
                $this->db->where($this->db->dbprefix('products') . ".supplier1", $company);
            }
            ...
            if (!empty($data)) {
                ...
                $this->excel->getActiveSheet()->SetCellValue('K1', lang('Precio unitario'));
                $this->excel->getActiveSheet()->SetCellValue('L1', lang('Cantidad'));
                $this->excel->getActiveSheet()->SetCellValue('M1', lang('Total'));
                $this->excel->getActiveSheet()->SetCellValue('N1', lang('Proveedor'));

                $this->excel->getActiveSheet()->SetCellValue('D14', lang('Resumen'));
                $this->excel->getActiveSheet()->SetCellValue('F13', lang('Total Contable'));
                $this->excel->getActiveSheet()->SetCellValue('G13', lang('Stock Total'));
                $row = 2;
                $sQty = 0;
                $pQty = 0;
                $sAmt = 0;
                ...
                $bAmt = 0;
                $pl = 0;
                $info_products = $this->reports_model->getProductsResum();
                foreach ($data as $data_row) {
                    ...
                    $this->excel->getActiveSheet()->SetCellValue('J' . $row, $data_row->cost);
                    $this->excel->getActiveSheet()->SetCellValue('K' . $row, $data_row->price);
                    ...
                    $this->excel->getActiveSheet()->SetCellValue('N' . $row, $data_row->company);
                ...
                $this->excel->getActiveSheet()->SetCellValue('F14' ,  $info_products->total);
                $this->excel->getActiveSheet()->SetCellValue('G14',$info_products->total_stock);
            ...
            $this->datatables
                ...
                CONCAT(COALESCE( PCosts.balacneQty, 0 ), '__', COALESCE( PCosts.balacneValue, 0 )) as balance, " . $this->db->dbprefix('products') . ".cost,  " . $this->db->dbprefix('products') . ".price,  " . $this->db->dbprefix('products') . ".quantity, ({$this->db->dbprefix('products')}.cost * {$this->db->dbprefix('products')}.quantity) as total, {$this->db->dbprefix('companies')}.company as company, {$this->db->dbprefix('products')}.id as id" , FALSE)
                ->from('products')
                ->join('companies', 'products.supplier1=companies.id', 'left')
            ...
            if ($company) {
                $this->datatables->where($this->db->dbprefix('products') . ".supplier1", $company);
            }
    ...
    function sales()
    {
         ...
         * Agregados
         */
        $this->data['paying_by'] = $this->payment_type_model->getPayments();
        $this->data['paying_actions'] = $this->payment_actions_model->getPaymentActions();
    ...
    function getSalesReport($pdf = NULL, $xls = NULL)
    {
        ...
        $payment_type =  $this->input->get('payment_type') ? $this->input->get('payment_type') : NULL;
        $paying_actions =  $this->input->get('paying_actions') ? $this->input->get('paying_actions') : NULL;
        if ($start_date) {
            $start_date = $this->sma->fld($start_date);
        ...
        if ($pdf || $xls) {
            $this->db
                ->select("
                    ...
                    com.name as clientes,
                    GROUP_CONCAT(CONCAT(" . $this->db->dbprefix('sale_items') . ".product_name, ' (', " . $this->db->dbprefix('sale_items') . ".quantity, ')') SEPARATOR '\n') as iname,
                    grand_total,
                    paid,
                    payment_status,
                    {$this->db->dbprefix('payment_type')}.name as pago,
                    {$this->db->dbprefix('payment_actions')}.name as venta", FALSE)
                ->from('sales')
                ->join('payments as p', 'p.sale_id=sales.id', 'left')
                ->join('users', 'users.id=sales.created_by', 'left')
                ->join('payment_type', 'payment_type.id=p.paid_by', 'left')
                ->join('payment_actions', 'payment_actions.id=p.paying_actions', 'left')
                ->join('sale_items', 'sale_items.sale_id=sales.id', 'left')
                ->join('warehouses', 'warehouses.id=sales.warehouse_id', 'left')
                ->join('companies as com','com.id=sales.customer_id')
                ->group_by('sales.id')
            ...
            if ($payment_type) {
                $this->db->where('payment_type.id', $payment_type);
            }

            if ($paying_actions) {
                $this->db->where('payment_actions.id', $paying_actions);
            }
            ...
            if (!empty($data)) {
                ...
                $this->excel->getActiveSheet()->SetCellValue('J1', lang('Pago'));
                $this->excel->getActiveSheet()->SetCellValue('K1', lang('Venta'));
                $row = 2;
                $total = 0;
                $paid = 0;
                $balance = 0;
                foreach ($data as $data_row) {
                    ...
                    $this->excel->getActiveSheet()->SetCellValue('D' . $row, $data_row->clientes);
                    ...
                    $this->excel->getActiveSheet()->SetCellValue('J' . $row, lang($data_row->pago!==null ?$data_row->pago:'Pendiente' ));
                    $this->excel->getActiveSheet()->SetCellValue('K' . $row, lang($data_row->venta!==null?$data_row->venta:'Pendiente' ));
            ...
            $this->load->library('datatables');
           /* $this->datatables
                ->select("DATE_FORMAT( {$this->db->dbprefix('sales')}.date, '%Y-%m-%d %T') as date,  {$this->db->dbprefix('sales')}.reference_no, CONCAT(" . $this->db->dbprefix('users') . ".first_name, ' ' ," . $this->db->dbprefix('users') . ".last_name) as biller, customer, FSI.item_nane as iname, grand_total, paid, (grand_total-paid) as balance, payment_status, {$this->db->dbprefix('payment_type')}.name, {$this->db->dbprefix('payment_actions')}.name as venta, {$this->db->dbprefix('sales')}.id as id", FALSE)
                ->from('sales')
                ->join('payments as p', 'p.sale_id=sales.id', 'left')
                ->join('users', 'users.id=sales.created_by', 'left')
                ->join('payment_type', 'payment_type.id=p.paid_by', 'left')
            ...
            $this->datatables
                ->select("DATE_FORMAT( {$this->db->dbprefix('sales')}.date, '%Y-%m-%d %T') as date,  {$this->db->dbprefix('sales')}.reference_no, CONCAT(" . $this->db->dbprefix('users') . ".first_name, ' ' ," . $this->db->dbprefix('users') . ".last_name) as biller, com.name as clientes, FSI.item_nane as iname, grand_total, paid, (grand_total-paid) as balance, payment_status,{$this->db->dbprefix('payment_type')}.name as t_pago, {$this->db->dbprefix('payment_actions')}.name as venta, {$this->db->dbprefix('sales')}.id as id", FALSE)
                ->from('sales')
                ->join('payments as p', 'p.sale_id=sales.id', 'left')
                ->join('users', 'users.id=sales.created_by', 'left')
                ->join('payment_type', 'payment_type.id=p.paid_by', 'left')
            ...
            if ($payment_type) {
                $this->datatables->like('payment_type.id', $payment_type);
            }

            if ($paying_actions) {
                $this->datatables->like('payment_actions.id', $paying_actions);
            }

...
class Sales extends MY_Controller
{
    ...
    public function getSales($warehouse_id = null)
    {
        ...
        if ($warehouse_id) {
            $this->datatables
                ->select("{$this->db->dbprefix('sales')}.id as id, DATE_FORMAT({$this->db->dbprefix('sales')}.date, '%Y-%m-%d %T') as date, reference_no, CONCAT(" . $this->db->dbprefix('users') . ".first_name, ' ' ," . $this->db->dbprefix('users') . ".last_name) as biller, com.name as clientes, sale_status, grand_total, paid, (grand_total-paid) as balance, payment_status, {$this->db->dbprefix('sales')}.attachment, return_id")
                ->from('sales')
                ->join('companies as com','com.id=sales.customer_id')
                ->join('users', 'users.id=sales.created_by', 'left') //MODIFICACION
    ...
    public function add($quote_id = null)
    {
        ...
        if ($this->form_validation->run() == true) {
            ...
            $data = array('date' => $date,
                ...
                //'paying_actions' =>  $this->input->post('paying_actions'),
                /*'payment_method' => ,
                'sell_type' => ,*/
            );
            ...
            if ($payment_status == 'partial' || $payment_status == 'paid' || $payment_status == 'pending' || $payment_status == 'due') {
    ...
    public function add_payment($id = null)
    {
        ...
        if ($this->form_validation->run() == true) {
            /*echo "<pre>";
            print_r($this->input->post('sale_id'));exit;*/
    ...
    /**
     * Retornar última detalle de venta
     */
    public function getLastSale() {
        $customer_id = (int) $this->input->get('customer');
        $sale = $this->db->query("SELECT * FROM sma_sales WHERE customer_id = {$customer_id} ORDER BY id DESC LIMIT 1");
        $sale_id = "";
        $aih = 0;
        ...
        $response = $this->db->query("SELECT
                        ...
                        where ssi.sale_id = {$sale_id}");

        foreach ($response->result() as $rsh) {
            $rows = $this->sales_model->getProductNames($rsh->product_name, $rsh->warehouse_id, 0);
            if ($rows) {
                $r = 0;
                foreach ($rows as $row) {
                    ...
                    $row->qty = $rsh->quantity;
                    $row->discount = $rsh->discount;
                    $row->serial = '';
                    $options = $this->sales_model->getProductOptions($row->id, $rsh->warehouse_id);
                    if ($options) {
                    ...
                    $pis = $this->site->getPurchasedItems($row->id, $rsh->warehouse_id, $row->option);
                    if ($pis) {
                        $row->quantity = 0;
                        foreach ($pis as $pi) {
                            $row->quantity += $pi->quantity_balance;
                    ...
                    $row->base_unit_price = $rsh->unit_price;
                    $row->unit = $row->sale_unit ? $row->sale_unit : $row->unit;
                    $row->comment = '';
                    $combo_items = false;
                    if ($row->type == 'combo') {
                        $combo_items = $this->sales_model->getProductComboItems($row->id, $rsh->warehouse_id);
                    }
                    $units = $this->site->getUnitsByBUID($row->base_unit);
                    $tax_rate = $this->site->getTaxRateByID($row->tax_rate);

                    $pr[$aih] = array('id' => sha1($c.$r), 'item_id' => $row->id, 'label' => $row->name . " (" . $row->code . ")", 'category' => $row->category_id,
                        'row' => $row, 'combo_items' => $combo_items, 'tax_rate' => $tax_rate, 'units' => $units, 'options' => $options);
                    $r++;
                    $aih++;
                }

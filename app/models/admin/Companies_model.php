...
class Companies_model extends CI_Model
{
    ...
    public function getCustomerSuggestions($term, $limit = 10)
    {
        $this->db->select("id, (CASE WHEN name = '-' THEN name ELSE CONCAT(name, ' (', address, ')') END) as text, (CASE WHEN name = '-' THEN address ELSE CONCAT(name, ' (', address, ')') END) as value, phone", FALSE);
        $this->db->where(" (id LIKE '%" . $term . "%' OR address LIKE '%" . $term . "%' OR name LIKE '%" . $term . "%' OR email LIKE '%" . $term . "%' OR phone LIKE '%" . $term . "%') ");
        $q = $this->db->get_where('companies', array('group_name' => 'customer'), $limit);
        if ($q->num_rows() > 0) {
            foreach (($q->result()) as $row) {
                $data[] = $row;

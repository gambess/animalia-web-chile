...
class Sales_model extends CI_Model
{
    ...
    {
        //añadi el payment_method para especificar el tipo de venta
        switch ($payment['paid_by'])
        {
            ...
            case 4:
                ...
                break;
            default:
                $data['payment_method']='Sin especificar';
        }
        //aqui agrego la venta
        switch ($payment['paying_actions'])
        {
            case 1 :
                $data['sell_type']='Fono Pedido';
                break;
            case 2 :
                $data['sell_type']='POS';
                break;
            case 3 :
                $data['sell_type']='Venta al por mayor';
                break;
            case 4:
                $data['sell_type']='Venta en línea';
                break;
            default:
                $data['sell_type']='Sin especificar';
        }
    ...
    {
        $this->db->select('payments.date, payments.paid_by, payments.amount, payments.cc_no, payments.cheque_no, payments.reference_no,payments.paying_actions, users.first_name, users.last_name, type,payment_type.name as tipo_pago,payment_actions.name as tipo_venta')
            ->join('users', 'users.id=payments.created_by', 'left')
        ->join('payment_type','payment_type.id=payments.paid_by')
        ->join('payment_actions','payment_actions.id=payments.paying_actions');
